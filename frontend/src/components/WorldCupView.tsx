...
import { Trophy, Swords, Download, RefreshCw, Zap, BarChart3, ZoomIn, ZoomOut, X } from 'lucide-react';
...
export default function WorldCupView() {
  ...
  // Modal State for Advanced Stats
  ...
  const openMatchStatsModal = async (matchId: string | number, homeId: number, awayId: number, homeName: string, awayName: string) => {
    ...
    if (matchStats[matchId]) return; // Already loaded
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="flex justify-between items-center bg-slate-900/80 p-4 rounded-xl border border-slate-800 mb-2">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2 bg-slate-950 p-1 rounded-lg border border-slate-800">
                <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-white" onClick={() => setBracketZoom(z => Math.max(0.3, z - 0.1))}>
                  <ZoomOut className="w-4 h-4" />
                </Button>
                <span className="text-xs font-mono w-10 text-center text-slate-300">{Math.round(bracketZoom * 100)}%</span>
                <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-white" onClick={() => setBracketZoom(z => Math.min(1.5, z + 0.1))}>
      ...
      {/* MATCH STATS MODAL (BETTING MARKETS) */}
      {selectedMatchModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <Card className="bg-slate-950 border-slate-800 w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            <div className="p-6">
              {loadingStats[selectedMatchModal.id] ? (
              ...
              ) : matchStats[selectedMatchModal.id] ? (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                      ...
                      <div className="p-4 grid grid-cols-3 gap-3">
                        ...
                        <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 hover:border-[#d4af37]/50 cursor-pointer transition-colors group">
                          ...
                          <div className="flex justify-between items-end">
                            <span className="text-2xl font-black text-[#f3e5ab]">{getOdds(matchStats[selectedMatchModal.id]['1x2']?.home_win)}</span>
                            <span className="text-xs text-green-400">{(matchStats[selectedMatchModal.id]['1x2']?.home_win * 100).toFixed(0)}%</span>
                          </div>
                        </div>
                        {/* X (Draw) */}
                        <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 hover:border-slate-600 cursor-pointer transition-colors group">
                          ...
                          <div className="flex justify-between items-end">
                            <span className="text-2xl font-black text-slate-300">{getOdds(matchStats[selectedMatchModal.id]['1x2']?.draw)}</span>
                            <span className="text-xs text-yellow-400">{(matchStats[selectedMatchModal.id]['1x2']?.draw * 100).toFixed(0)}%</span>
                          </div>
                        </div>
                        {/* 2 (Away) */}
                        <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 hover:border-blue-500/50 cursor-pointer transition-colors group">
                          ...
                          <div className="flex justify-between items-end">
                            <span className="text-2xl font-black text-blue-400">{getOdds(matchStats[selectedMatchModal.id]['1x2']?.away_win)}</span>
                            <span className="text-xs text-blue-400">{(matchStats[selectedMatchModal.id]['1x2']?.away_win * 100).toFixed(0)}%</span>
                          </div>
                        </div>
                      </div>
                    </Card>
                      ...
                      <Card className="bg-slate-900 border-slate-800 p-0 overflow-hidden">
                        ...
                        <div className="p-3 grid grid-cols-2 gap-2">
                          <div className="bg-slate-950 border border-slate-800 rounded p-2 text-center">
                            ...
                            <div className="font-bold text-[#f3e5ab]">{getOdds(matchStats[selectedMatchModal.id].btts?.yes)}</div>
                            <div className="text-[10px] text-slate-500">{(matchStats[selectedMatchModal.id].btts?.yes * 100).toFixed(0)}%</div>
                          </div>
                          <div className="bg-slate-950 border border-slate-800 rounded p-2 text-center">
                            <div className="text-[10px] text-slate-400 uppercase">No</div>
                            <div className="font-bold text-slate-300">{getOdds(matchStats[selectedMatchModal.id].btts?.no)}</div>
                            <div className="text-[10px] text-slate-500">{(matchStats[selectedMatchModal.id].btts?.no * 100).toFixed(0)}%</div>
                          </div>
                        </div>
                      </Card>
                      ...
                      <Card className="bg-slate-900 border-slate-800 p-0 overflow-hidden">
                        ...
                        <div className="p-3 grid grid-cols-2 gap-2">
                          <div className="bg-slate-950 border border-slate-800 rounded p-2 text-center">
                            ...
                            <div className="font-bold text-[#f3e5ab]">{getOdds(matchStats[selectedMatchModal.id].over_under?.['2.5']?.over)}</div>
                            <div className="text-[10px] text-slate-500">{(matchStats[selectedMatchModal.id].over_under?.['2.5']?.over * 100).toFixed(0)}%</div>
                          </div>
                          <div className="bg-slate-950 border border-slate-800 rounded p-2 text-center">
                            <div className="text-[10px] text-slate-400 uppercase">Menos de 2.5</div>
                            <div className="font-bold text-slate-300">{getOdds(matchStats[selectedMatchModal.id].over_under?.['2.5']?.under)}</div>
                            <div className="text-[10px] text-slate-500">{(matchStats[selectedMatchModal.id].over_under?.['2.5']?.under * 100).toFixed(0)}%</div>
                          </div>
                        </div>
                      </Card>
                    </div>
                    ...
                    <Card className="bg-slate-900 border-slate-800 p-0 overflow-hidden">
                      ...
                      <div className="p-4 flex gap-3 overflow-x-auto">
                        {Object.entries(matchStats[selectedMatchModal.id].correct_score_top5 || {}).map(([score, prob]: [string, any]) => (
                  ...
                  <div className="space-y-4">
                    ...
                    <Card className="bg-slate-900 border-slate-800 p-4">
                      <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Métricas del Simluador</h4>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-sm text-[#d4af37] w-8">{matchStats[selectedMatchModal.id].squad_rating?.home || "6.5"}</span>
                          <div className="flex-1 h-1.5 bg-slate-800 rounded-full flex">
                             <div className="bg-[#d4af37] h-full" style={{ width: `${(matchStats[selectedMatchModal.id].squad_rating?.home / 10) * 100}%` }} />
                          </div>
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                          <span className="font-mono text-sm text-blue-400 w-8">{matchStats[selectedMatchModal.id].squad_rating?.away || "6.5"}</span>
                          <div className="flex-1 h-1.5 bg-slate-800 rounded-full flex">
                             <div className="bg-blue-400 h-full" style={{ width: `${(matchStats[selectedMatchModal.id].squad_rating?.away / 10) * 100}%` }} />
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-sm text-[#d4af37] w-8">{matchStats[selectedMatchModal.id].expected_goals?.home || "1.2"}</span>
                          <div className="flex-1 h-1.5 bg-slate-800 rounded-full flex">
                             <div className="bg-[#d4af37] h-full" style={{ width: `${Math.min((matchStats[selectedMatchModal.id].expected_goals?.home / 3) * 100, 100)}%` }} />
                          </div>
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                          <span className="font-mono text-sm text-blue-400 w-8">{matchStats[selectedMatchModal.id].expected_goals?.away || "1.0"}</span>
                          <div className="flex-1 h-1.5 bg-slate-800 rounded-full flex">
                             <div className="bg-blue-400 h-full" style={{ width: `${Math.min((matchStats[selectedMatchModal.id].expected_goals?.away / 3) * 100, 100)}%` }} />
                          </div>
                        </div>
                      </div>
                    </Card>
                    ...
                    <Card className="bg-slate-900 border-slate-800 p-4">
                      ...
                      <div className="flex items-center justify-between py-2 border-b border-slate-800">
                        ...
                        <div className="text-right">
                          <div className="font-bold text-[#f3e5ab]">{matchStats[selectedMatchModal.id].corners?.total ? matchStats[selectedMatchModal.id].corners.total.toFixed(1) : '9.2'}</div>
                      ...
                      <div className="flex items-center justify-between py-2 border-b border-slate-800">
                        ...
                        <div className="text-right">
                          <div className="font-bold text-[#f3e5ab]">{matchStats[selectedMatchModal.id].cards?.total ? matchStats[selectedMatchModal.id].cards.total.toFixed(1) : '3.8'}</div>
