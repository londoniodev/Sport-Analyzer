...
import { Trophy, Swords, Download, RefreshCw, Zap, BarChart3, ZoomIn, ZoomOut } from 'lucide-react';
                              ...
                              <Card className={`bg-slate-900 border ${m.winner ? 'border-[#d4af37]/50 shadow-[0_0_10px_rgba(212,175,55,0.1)]' : 'border-slate-800'} overflow-hidden relative z-10 rounded-md`}>
                                ...
                                {!m.winner && m.home_team && m.away_team && (
                                  <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-[1px] flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                                    <Button size="sm" onClick={() => handleSimulateSingle(m.id)} disabled={simulatingBracket === m.id} className="bg-[#d4af37]/20 hover:bg-[#d4af37]/40 text-[#d4af37] border border-[#d4af37]/50 h-6 text-[10px] px-2 py-0">
